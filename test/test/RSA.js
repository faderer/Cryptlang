...
describe("RSA", function() {
    async function deployPermitFixture(){
        ...
        const rasver = await RSAVer.deploy();
        ...
        const nonce = await rasver.nonces(owner.address);
        const TYPEHASH = await rasver.TYPEHASH();
        const DOMAIN_SEPARATOR = await rasver.DOMAIN_SEPARATOR();
        ...
        return { rasver,signature,owner,spender,value,deadline,module,exponent };
    }
    describe("rsaver", function(){
        it("Shouldn't fail if upload the right value", async function () {
            const { rasver,signature,owner,spender,deadline,module,exponent } = await loadFixture(
                deployPermitFixture
            );
            // donot include value because stack too deep
            await expect(rasver.rasver(owner,spender,deadline,signature,exponent,module)).to.not.be.reverted;
        });
    });
});